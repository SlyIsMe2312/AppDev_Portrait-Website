...
@Component({
  ...
  template: `
    <nav class="navbar">
      <div class="container">
        <div class="nav-left">
          <a [routerLink]="brandRoute" class="navbar-brand">
...
export class NavbarComponent {
  brandRoute = '/';
  constructor(public auth: AuthService, private artistSvc: ArtistService) {}

  ngOnInit(): void {
    // default brand route based on role
    this.brandRoute = this.homeLink();
    const cur = this.auth.current();
    if (cur && cur.role === 'artist' && cur.email) {
      // attempt to resolve artist id by email and link to their public profile
      this.artistSvc.listArtists().subscribe((arr:any) => {
        const found = (arr || []).find((a:any) => a.email && a.email.toLowerCase() === (cur.email || '').toLowerCase());
        if (found) this.brandRoute = '/artists/' + found.id;
      }, _ => { /* ignore */ });
    }
  }
